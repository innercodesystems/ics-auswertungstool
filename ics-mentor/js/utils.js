...
ICS.trefferAnzahl = function(text,begriffe){
  return (begriffe||[]).reduce((summe,begriff)=>summe+(ICS.begriffGefunden(text,begriff)?1:0),0);
};

ICS.bestesErgebnis = function(text,daten){
  let bestes=null, bestePunktzahl=0;
  Object.keys(daten).forEach(function(id){
    ...
    if(punkte>bestePunktzahl){
      ...
      bestes=Object.assign({id,punkte},eintrag);
    }
  });
  return bestes;
};
