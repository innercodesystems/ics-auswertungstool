...
function senden() {
  ...
  setTimeout(function() {
    ...
    try {
      ...
      mentorNachricht(
        mentorAntwort ||
        "Ich möchte dich noch etwas besser verstehen. Was daran beschäftigt dich im Moment am stärksten?"
      );
      ...
      mentorNachricht(
        "Im Mentor ist gerade ein technischer Fehler aufgetreten. Deine Nachricht wurde dennoch aufgenommen."
      );
...
function dialogAntwort(
  ...
  if (prioritaet) {
    ...
    teile.push(
      `<em><strong>Aktueller Schwerpunkt:</strong> ${ICS.escapen(prioritaet.name)}</em>`
    );
  ...
  if (
    ...
    if (hypothesenFrage) {
      ...
      teile.push(
        `<strong>${ICS.escapen(hypothesenFrage)}</strong>`
      );
  ...
  if (frage) {
    ...
    teile.push(
      `<strong>${ICS.escapen(frage)}</strong>`
    );
...
function zusammenhang() {
  ...
  gespraech.weitereMuster.forEach(
    function(id) {
  ...
  return `Ich sehe inzwischen einen möglichen Zusammenhang zwischen <strong>${namen
    .map(ICS.escapen)
    .join(", ")}</strong>. Diese Aspekte wirken vermutlich nicht getrennt voneinander.`;
...
function zwischenSpiegel() {
  ...
  return `Dein roter Faden wird klarer: ${letzte
    .map(function(a) {
...
function unbekanntAntwort(analyse) {
  ...
  if (analyse.emotion) {
    ...
    )}<br><br><strong>Was ist unmittelbar davor passiert?</strong>`;
    ...
    muster.length
      ? muster
          .map(function(x) {
            ...
            return `${ICS.escapen(x.label)} <strong>${x.score}%</strong>`;
